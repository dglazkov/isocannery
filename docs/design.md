...
| 1 | the canvas | Open the tray and choose "Run on agents.isocan.io". The canvas mints a pass as you and sends you there with it. | 15 s |
| 2 | agents.isocan.io, once per canvas | One screen says who you are by name and id, that your space will now answer for the agents you add on this canvas, what that lets the service do, and where you turn it off. Then it sends you back. | 15 s |
| 3 | the canvas | Add an agent in the tray, name her, mention her. | 30 s |
...
There is no terminal and no key, and the second site is one screen, arrived at and left by a link. The predecessor's journey had eight steps across a browser and two command-line tools, and the person's identity had to survive four hand-offs. It broke at each one without saying so. Here there is one, the pass, and the screen that redeems it says aloud whose it is. The first version of step 1 is a paste: the person mints the pass under "Bring your own agent" and pastes it into their space. The tray's entry that does it in a redirect is isocan's to build.

Decided 18 Sep 2026: the owner of this project runs one service for everyone, agents.isocan.io, as a part of isocan and not a bridge beside it. A friend deploys nothing, needs no cloud account, and brings no key: the owner's Gemini key pays, inside a small free allowance per person. Each person has one space there: a machine of theirs that somebody else runs, which joins a canvas the way their laptop would, by redeeming a pass they minted ([isocan#333](https://github.com/dglazkov/isocan/issues/333), narrowed the same day).
...
| Host | this project | A Cloud Run service, holding one space per person. It hands each room its routes over `fetch`, under the badge that person's space holds for that canvas, its rows and state in Firestore, a `narrate` that writes the log, an `enrol` that gives a new agent her badge, and a turn that is the provider's start and continue. It counts the allowance and writes the timeline. |
...
| Secrets | In Secret Manager: the owner's Gemini key, the secret the room's agent keys derive from, and each space's badge for each canvas it joined, under its person's id | Read by the service's account alone. Never in a log line, a prompt or a document. |
...
| Sign-in to the page | Arriving with a pass says who you are, once. Coming back later is the person's isocan account | An ask on isocan, in #333. isocan's home provisions Google's Identity Platform; whether a sibling site can share that sign-in is to check. |
...
Identity is one rule, learned the hard way: every person and every agent is named with an id, and the service refuses aloud what it will not do. It says whose agent it is about to create before it creates her. The person who matters is the one whose space it is: they brought it to the canvas, their word wakes her, and their allowance pays. isocan does not ask that they own the canvas, only that they are admitted to it, and any admitted member may bring their own space.

Inside the sandbox she is herself only through a badge: there is no daemon and no machine secret there, and the CLI speaks to the home directly. Her badge never enters the sandbox. When she is enrolled the space mints a pass for her, as a holder of the person's claim may, and redeems it, with the `mintPass` and `redeemPass` that `isocan/rc` exports, and hands the badge to Google as a write-only credential trusted for isocan's domain; the sandbox sees only a placeholder that Google's egress proxy swaps on the way out. On the rig this is done by hand. The predecessor put the credential in the setup environment, and that one choice is what disabled its install cache.

### Custody

On a laptop, custody is one sentence of isocan's: you started the rc, so you own everything it runs. Hosted, the same sentence holds with one word changed, settled on [isocan#333](https://github.com/dglazkov/isocan/issues/333) on 18 Sep 2026: **a space is a machine of the person's that somebody else runs.** Each person has one. It joins a canvas by redeeming a pass that person minted there, and from then on it is to isocan what their laptop is: a badge, born from their pass, holding their claim. isocan learns no new kind of party, keeps no new record and gains no new rule.

- **A room exists because a person brought their space to the canvas.** The pass admits the space to that one canvas and hands it the person's claim, so the room's hold is believed to be theirs, and "add an agent" in the tray is routed to it. A second canvas is a second pass, into the same space.
- **There are two badges, and the agent is always herself.** Hers holds her actor only and lives at Google, write-only. Everything she does is in her own name, and if she named the person she would be refused. That is tighter than a laptop, where her process runs on the person's machine.
- **The space's badge is the person, on that canvas.** The room uses the person's claim for three things: the hold's owner, the room's announcement, and signing her enrolment when the person clicks Add. Everything else it says is her face or the system voice. But whoever holds that badge (the service, its owner, a thief of its store) could write as that person on that canvas, and isocan would accept it. What stands between is the room's code and the owner's word. Accepted for now by the owner. It is what a laptop's rc is, on somebody else's machine, and step 2's screen says so where the person decides. If it becomes uncomfortable, the way out is a narrower kind of pass, fixed at mint: hold the owner's label, enrol agents, answer for them, and write nothing else as the person.
- **The off switch lives in isocan.** The person ends the space's badge from isocan's identity menu, and it ends whether or not this service is up. Whether that also ends the agent badges the space passed in is unchecked, and is on #333. The room hears a withdrawal; the host then deletes her credential and her sandbox at Google, and her records here. The page offers the same switch and is never the only one.
- **What the owner of the service can do is said on the page.** Besides the badges: the Gemini project is theirs, so they can read what every agent did there for as long as Google keeps it.

### The key and the allowance

Decided 18 Sep 2026: the owner's Gemini key pays, and each person has a small free allowance, counted from the tokens on their turn records. Past it, the thread says so in a sentence. The room as built cannot say it: whatever a host throws becomes "couldn't answer… will be retried", every 60 s. A host needs to hold a turn in its own words with a time to try again, which is first on #333, and the allowance waits for it. So every sandbox, conversation and badge credential lives in one Gemini project, which the service can always clean up, and everyone's turns share that project's rate limits.
...
1. The room speaks first. Her face lands on the thread saying "reading your comment…" before her turn is started, with no model and no sandbox, so silence is never the first thing a person sees. That holds while one instance is always on. Once a summons is pushed to a service that may be asleep, the home puts her face up as it delivers, so a cold start costs the host and not the person, and the host's answer can take the face down with a reason (#333).
...
1. The room on Cloud Run for the owner alone: their space, joined to a canvas by a pasted pass, with the page that reads it. Walked from the browser, the person's clock read off the page. This is the mechanism everyone will use, not a stand-in for one. The first deploys also measure what a deploy does to a hold: holds are kept per badge, and one instance releasing ends every hold its badge has on the canvas, so an old instance stopping may cut the new one's.
2. What isocan is asked for, as one issue, [isocan#333](https://github.com/dglazkov/isocan/issues/333): the canvas, agent and owner by id on every turn, and a turn held in the host's own words; the tray's entry that mints the pass and redirects; that ending a space's badge ends its agents' badges; what two instances do; a summons delivered to a URL, with the home putting her face up; sign-in for the page.
3. The allowance and its sentence on the thread, once a host can say it.
...
| How isocan trusts the host | Decided 18 Sep 2026, on #333: it does not. A person's space redeems a pass they minted, and is to isocan what their laptop is. One space per person, one badge per canvas. | A host isocan knows by name, with a consent record, was this design's first answer and was dropped as more than the job needs. The power a space's badge carries is accepted for now; a narrower pass is the way out. |
| How a summons arrives | The room's held request, which needs an instance always on. Or a delivery from the home to a URL, so one request is one turn and the service sleeps. | The held request first, because it needs nothing from isocan. The delivery is last on #333. It fits isocan's 30 Aug 2026 decision for an address over a dispatch hook: that turned on a hook answering 204 and saying nothing, and a URL that answers "accepted" or a refusal in words does not. |
| May two instances run | Read in isocan's source on #333: never refused. Cursors are per actor, so two would answer twice or turn each other away, and one stopping releases the other's hold. | One instance, and measure it before anything is designed around it. |
