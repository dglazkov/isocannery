...
| How a summons reaches the bridge | Decided 18 Sep 2026: through `isocan rc`, which starts the bridge as an ACP adapter. No `isocan wait` loop and no webhook of the bridge's own. | It needed no change in isocan: `acpAdapters` already takes any command. Walked 18 Sep 2026 with a fake turn and no model, the rc in a codespace joined through "Bring your own agent": an agent added in the canvas's tray with no harness named ran on the adapter, the rc's log has 2 s from summons to the end of the turn, and the second mention resumed the first one's session in a fresh adapter process. |
